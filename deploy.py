...
"""
    ...
    # Run database migrations (safe ALTER TABLE operations)
    print("--- Running database migrations... ---")
    execute_command(ssh, f"cd {remote_path} && ./venv/bin/python migrate_db.py")
    ...
    # Stop bot first, clean Telegram session, then restart
    print("--- Stopping bot for clean restart... ---")
    execute_command(ssh, "systemctl stop gora_bot")
    execute_command(ssh, "pkill -f 'bot.main' || true")
    
    # Call Telegram close() API to clear server-side sessions
    print("--- Cleaning Telegram session... ---")
    execute_command(ssh, f"cd {remote_path} && ./venv/bin/python -c \"import requests; import os; from dotenv import load_dotenv; load_dotenv(); token=os.getenv('TELEGRAM_BOT_TOKEN'); r=requests.post(f'https://api.telegram.org/bot{{token}}/close'); print('close:', r.json()); r=requests.post(f'https://api.telegram.org/bot{{token}}/deleteWebhook', json={{'drop_pending_updates': True}}); print('deleteWebhook:', r.json())\"")
    
    import time
    print("--- Waiting 15 seconds for Telegram to release... ---")
    time.sleep(15)
    
    # Now restart services
    execute_command(ssh, "systemctl restart gora_admin")
    execute_command(ssh, "systemctl start gora_bot")
