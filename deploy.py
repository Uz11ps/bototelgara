...
"""
    ...
    # Run DB migration (safe to run multiple times)
    print("--- Running DB migration... ---")
    execute_command(ssh, f"cd {remote_path} && ./venv/bin/python scripts/add_scheduled_at.py")
    execute_command(ssh, f"cd {remote_path} && ./venv/bin/python scripts/add_start_notification_sent.py")
