...
const App: React.FC = () => {
  ...
  const [isTelegram, setIsTelegram] = useState(true); // Assume true initially to avoid flicker
  const [webApp, setWebApp] = useState<any>(null);

  useEffect(() => {
    // 1. Try to get WebApp from window
    const app = getWebApp();
    ...
    if (app) {
      ...
      if (tgUser && tgUser.id) {
        ...
        // WebApp exists but no user (e.g. external browser with script loaded, or weird state)
        // Check if we are in a mock environment (dev)
        if (process.env.NODE_ENV === 'development') {
          setUser({ id: 1, first_name: 'Dev', last_name: 'User' });
          setIsTelegram(true);
        } else {
          console.warn("Telegram WebApp detected but no user data found.");
          setIsTelegram(false);
        }
      }
    } else {
      // WebApp not found at all
      if (process.env.NODE_ENV === 'development') {
        setUser({ id: 1, first_name: 'Dev', last_name: 'User' });
        setWebApp({ close: () => { } }); // Mock
        setIsTelegram(true);
      } else {
        setIsTelegram(false);
      }
  ...
  if (!isTelegram) {
    return (
      <div className="min-h-screen bg-[#fdfcf7] flex flex-col items-center justify-center p-8 text-center animate-fade-in">
        <div className="text-6xl mb-6">📱</div>
        <h1 className="text-2xl font-extrabold text-emerald-900 mb-4">Откройте в Telegram</h1>
        <p className="text-slate-600 mb-6">
          Это приложение работает только внутри Telegram бота отеля.
        </p>
        <a
          href="https://t.me/Gora_Hotel_Bot"
          className="bg-emerald-600 text-white px-6 py-3 rounded-xl font-bold hover:bg-emerald-700 transition shadow-lg shadow-emerald-200"
        >
          Перейти в бот
        </a>
      </div>
      ...
      <nav className="fixed bottom-0 left-0 right-0 glass-nav px-2 pt-1 pb-[env(safe-area-inset-bottom,8px)] flex justify-around shadow-[0_-4px_30px_rgba(0,0,0,0.08)] z-40">
        {([
          ...
          <button
            ...
            className={`flex-1 flex flex-col items-center py-2.5 rounded-2xl transition-all duration-300 ${activeTab === tab.id
