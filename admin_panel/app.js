...
class AdminApp {
    ...
    async saveMenuItemWithComposition() {
        ...
        if (fileInput && fileInput.files && fileInput.files[0]) {
            const uploaded = await this.uploadImage('menuImageFile');
            if (uploaded) imageUrl = uploaded;
        } else {
            // Check if URL was typed manually
        ...
        container.innerHTML = this.tickets.map(t => `
            <div onclick="app.loadTicketDetail(${t.id})" class="bg-white p-4 rounded-xl shadow hover:shadow-md cursor-pointer border-l-4 ${t.status === 'PENDING_ADMIN' ? 'border-orange-500' : 'border-gray-200'} transition">
                ...
                <div class="text-sm text-gray-600 mt-2">
                    <div>👤 ${t.guest_name && t.guest_name !== 'None' ? t.guest_name : (t.guest_chat_id || 'Гость')} <span class="text-gray-400 text-xs">(${t.guest_chat_id})</span></div>
        ...
        return `
            <div class="bg-blue-50 p-6 rounded-xl border-2 border-blue-200 mb-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="md:col-span-2">
                        <label class="block text-sm font-bold mb-2">Изображение</label>
                        ${m.image_url ? `<div class="mb-2"><img src="${m.image_url}" class="w-32 h-20 object-cover rounded border" onerror="this.style.display='none'"></div>` : ''}
                        <div class="flex gap-2 items-end">
                            <div class="flex-1">
                                <label class="block text-xs text-gray-500 mb-1">Загрузить файл</label>
                                <input type="file" id="menuImageFile" accept="image/*" class="w-full border p-2 rounded bg-white text-sm">
                            </div>
                            <div class="flex-1">
                                <label class="block text-xs text-gray-500 mb-1">Или URL изображения</label>
                                <input id="menuImageUrl" class="w-full border p-2 rounded" value="${m.image_url || ''}" placeholder="https://...">
                            </div>
                        </div>
                    </div>
                </div>
    ...
    renderStaffTab() {
        return `<div class="space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ${this.staffTasks.map(s => `
                    <div class="bg-white p-4 rounded-xl shadow border-l-4 ${s.status === 'PENDING' ? 'border-red-500' : 'border-green-500'} hover:shadow-lg transition">
                        <div class="flex justify-between items-start mb-2">
                            <div class="flex items-center gap-2">
                                <span class="font-bold text-lg">#${s.room_number || '---'}</span>
                                <span class="text-xs px-2 py-1 bg-gray-100 rounded-full">${s.task_type}</span>
                            </div>
            ...
            <!-- Modal for New Task -->
            <div id="taskModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50 backdrop-blur-sm">
                <div class="bg-white p-6 rounded-2xl w-full max-w-lg shadow-2xl transform transition-all scale-100">
                    <div class="flex justify-between items-center mb-6">
                        <h3 class="text-xl font-bold">Новая задача</h3>
                        <button onclick="app.hideModal()" class="text-gray-400 hover:text-gray-600">✕</button>
                    </div>
                    
                    <div class="space-y-4">
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label class="block text-sm font-bold mb-1">Номер комнаты</label>
                                <input id="taskRoom" class="w-full border p-2.5 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" placeholder="Напр. 101">
                            </div>
                            <div>
                                <label class="block text-sm font-bold mb-1">Тип задачи</label>
                                <select id="taskType" onchange="app.toggleTaskFields()" class="w-full border p-2.5 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none bg-white">
                                    <option value="cleaning">🧹 Уборка</option>
                                    <option value="technical_problem">🔧 Тех. проблема</option>
                                    <option value="food_to_room">🍽 Еда в номер</option>
                        ...
                        <div>
                            <label class="block text-sm font-bold mb-1">Время выполнения (необязательно)</label>
                            <input id="taskScheduledTime" type="datetime-local" class="w-full border p-2.5 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none">
                            <div class="text-xs text-gray-500 mt-1">Ответственный получит уведомление за 15 минут</div>
                        </div>

                        <!-- Conditional Fields -->
                        <div id="foodFields" class="hidden grid grid-cols-2 gap-4 bg-orange-50 p-3 rounded-lg border border-orange-100">
                            <div>
                                <label class="block text-sm font-bold mb-1 text-orange-800">Время</label>
                                <input id="taskTime" type="time" class="w-full border p-2 rounded-lg">
                            </div>
                            <div>
                                <label class="block text-sm font-bold mb-1 text-orange-800">Кол-во персон</label>
                                <input id="taskQty" type="number" min="1" value="1" class="w-full border p-2 rounded-lg">
                            </div>
                        </div>

                        <div>
                            <label class="block text-sm font-bold mb-1">Описание / Детали</label>
                            <textarea id="taskDesc" class="w-full border p-3 rounded-lg h-24 focus:ring-2 focus:ring-blue-500 outline-none resize-none" placeholder="Дополнительная информация..."></textarea>
                        </div>
                        ...
                        <div class="flex gap-3 mt-6">
                            <button onclick="app.submitStaffTask()" class="flex-1 bg-blue-600 text-white py-3 rounded-xl font-bold hover:bg-blue-700 transition shadow-lg shadow-blue-200">Создать задачу</button>
                            <button onclick="app.hideModal()" class="px-6 border py-3 rounded-xl hover:bg-gray-50 transition">Отмена</button>
                        </div>
                    </div>
                </div>
            </div>
    ...
    showCreateStaffTask() {
        ...
        this.toggleTaskFields(); // Init state
    }
    ...
    async submitStaffTask() {
        ...
        if (!room) return alert('Укажите номер комнаты');
        if (!resp) return alert('Выберите ответственного сотрудника');

        // Append structured data to description if applicable
        if (type === 'food_to_room') {
            const time = document.getElementById('taskTime').value;
            const qty = document.getElementById('taskQty').value;
            if (!time) return alert('Укажите время подачи');
            desc = `[Еда в номер]\nВремя: ${time}\nКол-во: ${qty}\n\nКомментарий: ${desc}`;
        }
    ...
    async deleteStaffTask(taskId) {
        if (!confirm('Удалить задачу?')) return;
        try {
            await fetch(`${API_BASE}/staff/tasks/${taskId}`, { method: 'DELETE' });
            this.loadStaffTasks();
        } catch (e) {
