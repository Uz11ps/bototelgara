...
async def _handle_in_house_logic(message: Message, state: FSMContext, telegram_id: str):
    ...
    from bot.keyboards.main_menu import build_in_house_menu, build_guest_booking_keyboard, build_main_reply_keyboard
    await state.set_state(FlowState.in_house_menu)
    await message.answer(text, parse_mode="HTML", reply_markup=build_main_reply_keyboard())
